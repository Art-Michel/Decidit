...
public class FlyAI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // si la position al�atoir de patroll est d�fini l'IA si dirige, sinon on en cherche une
        if (newPosIsSet)
        {
        ...
        // si un obstacle est d�tect� est que l'IA n'est pas assez loin on cherche la direction de la destination (droite ou gauche) pour savoir de quel cot� contourn� l'obstacle
        if (obstacleOnPathDetected)
        {
    ...
    ////////////// Contourne Obstacle \\\\\\\\\\\\\\\\\\\\\
    void CheckDestinationDir() // si la destination est a droite ou � gauche, lance la fonction "ContourneDirection()"
    {
        if (dirContourne > 0)
        {
